...
- **Citation:** metadata are given in `CITATION.cff` (read by GitHub's "Cite this repository") and in `.zenodo.json`.
- **License:** the code is released under the MIT License (`LICENSE`), and the numerical results and figures under CC BY 4.0.
- **Archive:** the Zenodo archive also contains the generated figures and the compiled article. Its DOI is entered in `manuscript/main.tex` (`\zenodoDOI`) once it has been reserved.
